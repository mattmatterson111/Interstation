...
/obj/machinery/computer/slot_machine/attackby(obj/item/I, mob/living/user, params)
	if(istype(I, /obj/item/stack/spacecash))///obj/item/weapon/coin))
		...
		if(prob(2))
			...
			if(!user.drop_item())
				return
			user << "<span class='notice'>You insert [C.value] credits into [src]'s slot!</span>"//a [C.cmineral] coin
