...
/obj/item/weapon/storage/box/matches/attackby(obj/item/weapon/match/W as obj, mob/user as mob, params)
	if(istype(W, /obj/item/weapon/match))
		if(W.lit)
			return
		if(prob(25))//No more instant 1 strike matches.
			user.visible_message("<span class='notice'>[user] strikes \the [src] with the match and it lights!</span>")
			playsound(src, 'sound/items/matchstick_lit.ogg', 40, 1, -1)
			W.matchignite()
		if(prob(20))//Too much?
			user.visible_message("<span class='notice'>[user] strikes \the [src] too hard and breaks the match!</span>")
			W.lit = TRUE
			W.matchburnout()
		else
			user.visible_message("<span class='notice'>[user] strikes \the [src] with the match.</span>")
			playsound(src, 'sound/items/matchstick_hit.ogg', 40, 1, -1)
